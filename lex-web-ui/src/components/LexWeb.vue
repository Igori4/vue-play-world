<template>
  ...
  >
    ...
    <v-content>
      <v-container :class="messageListClasses" fluid >
...
export default {
  ...
  computed: {
    ...
    messageListClasses() {
      if (this.isDrawerOpened) {
        return 'message-list-container message-list-container-sidebar';
      }
      return 'message-list-container';
    },
  },
  watch: {
  ...
  async beforeMount() {
    try {
      ...
      const userAttributes = await this.$store.dispatch('getUserAttributes');
      let suggestionsList = [];
      try {
        let userIntents = userAttributes['custom:user_intents'];
        if (userIntents) {
          userIntents = JSON.parse(userIntents);
        }
        if (userIntents && userIntents.constructor === Array && userIntents.length > 0) {
          suggestionsList = userIntents;
        } else {
          suggestionsList = this.$store.state.config.Suggestions;
        }
      } catch (e) {
        suggestionsList = this.$store.state.config.Suggestions;
      }
      ...
      const userSessionAttributes = {
        ...
        ref_param: userAttributes['custom:ref_param'],
        client: 'web_bot',
      ...
      this.$store.dispatch('postInitialSessionTextMessage');
    } catch (e) {
      console.error(e);
      try {
  ...
  created() {
    ...
    this.initConfig()
      ...
      .then(() => this.$store.dispatch('getUserAttributes'))
      .then((userAttributes) => {
        this.$store.dispatch('setUserId', userAttributes.sub);
...
.message-list-container-sidebar {
