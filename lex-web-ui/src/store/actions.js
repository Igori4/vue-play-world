...
let audio;
let recorder;
let lastResponseTime = new Date().getTime();
...
export default {
  ...
  resetSessionIfNeeded(context) {
    return Promise.resolve()
      .then(() => (
        ((((context.state.lex.dialogState === 'Fulfilled') ||
          (context.state.lex.dialogState === 'Failed')) &&
          (context.state.config.lex.reInitSessionAttributesOnRestart === true)) ||
          (context.state.config.lex.reInitSessionAttributesTimeOut > 0 &&
            ((new Date().getTime() - lastResponseTime) >
              (context.state.config.lex.reInitSessionAttributesTimeOut * 1000)))
        ) ?
          context.commit(
            'setLexSessionAttributes',
            context.state.config.lex.sessionAttributes,
  ...
  postInitialSessionTextMessage(context) {
    return context.dispatch('interruptSpeechConversation')
      .then(() => context.dispatch('lexPostText', 'initiating a new session'))
      .then((response) => {
  ...
  postTextMessageRefreshToken(context) {
    if (!awsCredentials.needsRefresh()) {
      return Promise.resolve();
    }
    return new Promise((resolve, reject) => {
      awsCredentials.getPromise()
        .then(() => context.dispatch('getCurrentUser'))
        .then(() => {
          const userAttributes = {
            client: 'web_bot',
            source_url: window.location.href,
            access_token: context.state.cognito.user.tokens.AccessToken,
          };
          context.commit('setConfigSessionAttributes', userAttributes);
          context.commit('setLexSessionAttributes', userAttributes);
        })
        .then(() => resolve())
        .catch((err) => {
  ...
  async postTextMessage(context, message) {
    return context.dispatch('interruptSpeechConversation')
      .then(() => context.dispatch('postTextMessageRefreshToken'))
      .then(() => context.dispatch('resetSessionIfNeeded'))
      ...
      .then((response) => {
        ...
        if (response.message.includes('{"messages":')) {
          ...
          if (response.sessionAttributes.render_objects) {
            renderObjects = JSON.parse(response.sessionAttributes.render_objects);
          }
          context.dispatch(
            'pushMessage',
            {
              ...
              type: 'bot',
              dialogState: context.state.lex.dialogState,
              intentName: context.state.lex.intentName,
              responseCard: context.state.lex.responseCard,
              alts,
              renderObjects,
            },
      ...
      .then(() => {
        if (!context.state.lex.intentName) {
          axios.post('https://25x0ngbkub.execute-api.us-east-1.amazonaws.com/api/missed_utters', {
            user_id: context.state.cognito.user.username,
            missed_utter: message.text,
          }).then(() => {
            console.info('missed_utter posted');
          }).catch(() => {
            console.error('missed_utter failed to post');
          });
        }
      })
      .then(() => {
  ...
  pushMessage(context, message) {
    if (!(message.visibility && message.visibility === 'hide')) {
  ...
  setUserId(context, id) {
    lexClient.initUserId(id);
  },
  applicationStateDefault(context) {
    context.commit('applicationStateDefault');
  },
  putInRequest(context) {
    context.commit('putInRequest');
  },
  putInError(context, error) {
    let payload = '';
    if (context.state.config.CognitoMessages[error.code]) {
      payload = context.state.config.CognitoMessages[error.code];
    } else {
      payload = error.message;
    }
    context.commit('putInError', payload);
  },
  putInSuccess(context, key) {
    let payload = '';
    if (context.state.config.CognitoMessages[key]) {
      payload = context.state.config.CognitoMessages[key];
    } else {
      payload = 'Success!';
    }
    context.commit('putInSuccess', payload);
  },
  setSuggestionsList(context, suggestions) {
    context.commit('setSuggestionsList', suggestions);
  },
};
